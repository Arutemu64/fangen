...
1. Установить [uv](https://docs.astral.sh/uv/) — единственный поддерживаемый
   способ запуска.
...
3. Переименовать `config.dist.toml` → `config.toml` и заполнить актуальными данными:
...
Открыть терминал в каталоге с репозиторием и запустить:
